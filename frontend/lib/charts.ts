...
const Charts = (function () {
  ...
  function base(t) {
    return {
      ...
      tooltip: {
        trigger: 'axis', backgroundColor: t.tipBg, borderColor: t.tipLine,
  ...
  function build(result) {
    const type = (result.chart || 'bar').toLowerCase();
    if (type === 'table' || (!result.labels || !result.labels.length)) return null;
    const t = tokens();
  ...
  function line(t, result) {
    const series = (result.series || []).map((s, i) => ({
      ...
      symbol: 'circle',
      symbolSize: 6,
      showSymbol: (result.labels || []).length <= 40,
      data: s.data,
      lineStyle: { width: 2.4 },
      areaStyle: result.series.length === 1 ? { opacity: t.area, color: t.palette[i % t.palette.length] } : undefined,
      ...
      markPoint: result.anomalyPoints && result.anomalyPoints.length ? {
        ...
        label: { fontSize: 10, color: '#fff', formatter: (p) => p.value != null ? Number(p.value).toLocaleString('zh-CN', { maximumFractionDigits: 1 }) : '' },
        data: result.anomalyPoints.slice(0, 12).map((p) => ({ coord: [p.label, p.value], value: p.value })),
      } : undefined,
    }));

    return Object.assign({}, base(t), {
      legend: result.series.length > 1 ? { top: 8, textStyle: { color: t.ink, fontSize: 12 } } : undefined,
      xAxis: Object.assign({ type: 'category', boundaryGap: false, data: result.labels, axisLabel: { color: t.ink2, fontSize: 11, hideOverlap: true, rotate: result.labels.length > 12 ? 35 : 0 } }, axis(t)),
      yAxis: Object.assign({ type: 'value' }, axis(t, '')),
      dataZoom: (result.labels || []).length > 30 ? [{ type: 'inside' }, { type: 'slider', height: 16, bottom: 14, borderColor: t.line, fillerColor: 'rgba(91,91,214,.18)', handleStyle: { color: '#5b5bd6' } }] : undefined,
      series,
    });
  }

  function bar(t, result) {
    return Object.assign({}, base(t), {
      legend: result.series.length > 1 ? { top: 8, textStyle: { color: t.ink, fontSize: 12 } } : undefined,
      xAxis: Object.assign({
        type: 'category',
        data: result.labels,
        axisLabel: { color: t.ink2, fontSize: 11, hideOverlap: true, interval: 0, rotate: result.labels.length > 8 ? 30 : 0 },
      }, axis(t)),
      yAxis: Object.assign({ type: 'value' }, axis(t, '')),
  ...
  function pie(t, result) {
    const s = (result.series || [])[0];
    return Object.assign({}, base(t), {
      tooltip: { trigger: 'item', backgroundColor: t.tipBg, borderColor: t.tipLine, textStyle: { color: t.tipInk }, formatter: '{b}: {c} ({d}%)' },
      legend: { type: 'scroll', orient: 'vertical', right: 8, top: 20, textStyle: { color: t.ink, fontSize: 12 } },
      series: [{
        name: s ? s.name : '',
        type: 'pie',
        radius: ['42%', '68%'],
  ...
  function scatter(t, result) {
    const sc = result.scatter;
    if (!sc) return bar(t, result);
    return Object.assign({}, base(t), {
      tooltip: { trigger: 'item', backgroundColor: t.tipBg, borderColor: t.tipLine, textStyle: { color: t.tipInk }, formatter: (p) => `${sc.x}: ${p.data[0]}<br/>${sc.y}: ${p.data[1]}` },
      xAxis: Object.assign({ type: 'value', scale: true, name: sc.x }, axis(t, sc.x)),
      yAxis: Object.assign({ type: 'value', scale: true, name: sc.y }, axis(t, sc.y)),
      series: [{
        type: 'scatter',
        symbolSize: 7,
  ...
  function rerenderAll() {
    const boxes = document.querySelectorAll ? document.querySelectorAll('.chart-box') : [];
    Array.prototype.forEach.call(boxes, (el) => { if (el.__result) render(el, el.__result); });
  }
